<x-layouts.layout title="Category - {{ ucwords($category) }}">
    <div class="mt-8">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach ($questions as $question)
                            ...
                            <a href="{{ route('question-show', [$question->slug]) }}" target="_blank" class="relative p-4 border shadow-sm space-y-1 rounded-md hover:shadow-lg border-gray-400 hover:border-gray-800">
                                <p class="abosulte w-fit px-2 py-1 -ml-6 space-x-2 -mt-3 text-xs bg-gray-200">
                                ...
                                @foreach ($categoryLogo as $logo => $src)
                                    @if ($logo == $category)
                                        <img class="p-12" src="{{ $src }}" alt="{{ $category }}_logo">
                                    @endif
                                @endforeach
